...
public class MovementHamsterBall2 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        for (uint i = 0; i < trackThese.Length; i++)
        {
            ...
            Vector3 headPosition = transform.parent.GetComponentInChildren<Camera>().transform.position;
            Vector3 controllerPosition = checkMe.transform.position;
            float headDistance = Vector3.Distance(headPosition, transform.position);
            float radius = this.GetComponent<SphereCollider>().radius;
            float angle = Vector3.Angle(headPosition, transform.position);
